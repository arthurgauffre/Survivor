...
def fetchSingleEvent(event_data, access_token):
    ...
    try:
        if event_details:
            existing_event = session.query(Events).filter(Events.id == event_data.get('id')).first()
